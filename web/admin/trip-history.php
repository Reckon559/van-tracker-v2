...
$trips = database()->query(
    'SELECT t.id, t.trip_type, t.status, t.created_at,
            ...
            t.predicted_eta_sec, r.name AS route_name,
            v.van_number, v.plate_number, u.name AS driver_name,
...
function format_trip_duration(int $seconds): string
{
    if ($seconds <= 0) return '—';
...
function format_trip_timestamp(?string $value): string
{
    return $value ? date('M d, Y · H:i', strtotime($value)) : '—';
}

render_header('Trip history');
...
<div class="page-heading">
    <div>
        ...
        <h1>Trip history</h1>
        <p>All scheduled, running, completed and cancelled trips.</p>
    </div>
    <a class="primary-button" href="<?= APP_BASE_URL ?>/admin/manage-trips.php">Create trip</a>
</div>
<section class="panel">
    <?php if (!$trips): ?>
        <p class="muted">No trip records are available.</p>
    <?php else: ?>
        <div class="history-toolbar">
            <label>
                <span>Search trips</span>
                <input id="trip-history-search" type="search"
                       placeholder="Route, driver, van or trip number">
            </label>
            <label>
                <span>Status</span>
        ...
        <div class="data-table-wrap history-table-wrap"><table class="data-table history-table">
            <thead><tr>
                ...
                <th>Status</th><th>Attendance</th><th>Duration</th>
            ...
            <tbody><?php foreach ($trips as $trip): ?>
                <tr data-trip-history-row
                    ...
                    data-search="<?= escape(strtolower(implode(' ', [
                        ...
                        $trip['driver_name'], $trip['van_number'], $trip['plate_number'],
                    ]))) ?>">
                    ...
                    <td><?= escape($trip['driver_name']) ?></td>
                    <td><span class="status-badge status-<?= escape($trip['status']) ?>"><?= escape(ucfirst($trip['status'])) ?></span></td>
                    <td>
                        <?php if ($trip['trip_type'] === 'morning'): ?>
                            Not required
                        <?php else: ?>
                            <?= (int) $trip['present_count'] ?> present ·
                            <?= (int) $trip['absent_count'] ?> absent
                            <?php if ((int) $trip['unmarked_count'] > 0): ?>
                                <br><small><?= (int) $trip['unmarked_count'] ?> unmarked</small>
                            <?php endif; ?>
                        <?php endif; ?>
                    </td>
                    <td><?= escape(format_trip_duration((int) $trip['simulated_elapsed_sec'])) ?></td>
                    <td class="history-timeline">
                        <span>Created: <?= escape(format_trip_timestamp($trip['created_at'])) ?></span>
                        <span>Started: <?= escape(format_trip_timestamp($trip['started_at'])) ?></span>
                        <span>Finished: <?= escape(format_trip_timestamp($trip['completed_at'])) ?></span>
                    </td>
